...
class Embeds(commands.Cog):
    ...
    @commands.group(
        name='BotSay',
        description="Turn a bot into you",
        brief="Say anything!",
        aliases=['botsay', 'say'],
    ...
    @say.command(
        name='Edit',
        description="Edit an old message!",
        brief="Edit bot message",
        aliases=['edit', 'e'])
    @commands.guild_only()
    @commands.is_owner()
    async def edit_say(self, ctx, message: discord.Message, *, content: str=None):
        ...
        if message.author != self.bot.user:
            await ctx.channel.send(f"{message.author.mention} Sorry, The message is not mine !!")
            return
    ...
    @commands.command(
        name='DailyCredits',
        aliases=['spin', 'daily', 'credits', 'cashme', 'getmoney'])
    @commands.guild_only()
    @checks.check_perms()
    async def daily_cash(self, ctx):
        ...
        if timesince < delta:
            ...
            if hours:
                str = (f"You've already used your loan for the day, {member.mention}! "
                       f"You'll have to wait {hours} hour(s) and "
                       f"{minutes} minute(s) to loan again.")
            elif seconds > 60:
                str = (f"You've already used your loan for the day, {member.mention}! "
                       f"You'll have to wait {minutes} minute(s) "
                       f"and {seconds} second(s) to loan again.")
            else:
                str = (f"You've already used your loan for the day, {member.mention}! "
                       f"You'll have to wait {seconds} second(s) to loan again.")
            await ctx.channel.send(str)
        else:
            cashy = random.choice([30, 100, 200, 500, 1000])
            newcash = dbprof['cash'] + cashy
            await db.set_member(gID, member.id, 'cash', newcash)
            await db.set_member(gID, member.id, 'daily_timestamp', now)
            await ctx.channel.send(
                f"{cashy} credits has been transfered to your credit, {member.mention}!")
    ...
    async def transfer(self, ctx, amount: int, member: discord.Member):
        if member.bot:
            await ctx.channel.send("Bots are bots, they dont have a profile!")
            return
        if amount < 1:
            await ctx.channel.send(
        ...
        if ctx.author == member:
            await ctx.channel.send("Poor, sadly can't transfer credits to yourself.")
            return
        dbprof_author = await db.get_member(ctx.author.guild.id, ctx.author.id)
        if amount > dbprof_author['cash']:
            await ctx.channel.send(
                f"**{ctx.author.mention} You're too poor to transfer, to check your credit use (-porfile) command!**")
            return
    ...
    async def transfer_handler(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            if error.param.name == 'amount':
                ...
                await ctx.send(
                    "error- mention a member you wish to transfer!!")
                return
    ...
    async def award(self, ctx, amount: int, member: discord.Member):
        ...
        if amount < 1:
            await ctx.channel.send(
                f"**{ctx.author.mention} Poor huh? Sadly the lowest amount you can transfer is 1**")
            return
        ...
        if controls:
            ...
            while not bought:
                ...
                # If none of selected item are available, throw error to user
                    ...
                    await ctx.channel.send(f"{ctx.message.author.mention} {Shop.selected['fname']} is not currently available. Please select another item.")
                    return
            ...
            if Shop.selected['prompt2']:
                await ctx.send(f"**{Shop.selected['fname']} bought! Check your DM's.**")
                #await message.edit(content=edit, delete_after=5.0)
                await db.add_temp(
                    ctx.guild.id,
                    ctx.author.id,
    ...
    async def purchase(self, message, selected, temp):
        if selected['data'] == 'role':
            if not temp['storage']:
                ...
                embed = discord.Embed(title=(f"CONGRATS!!!!"), description= "Role Created and purchased!! ",color=0xfff700)
                embed.set_image(url="https://media.giphy.com/media/26tPplGWjN0xLybiU/giphy.gif")
                embed.add_field(name="تم تعيين الرتبه", value="اضغط على حسابك في السيرفر للتحقق", inline=True)
